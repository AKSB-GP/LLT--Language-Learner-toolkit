...
(() => {
  ...
  var NotificationView = class {
    ...
    promptTargetLanguage(options) {
      const targetOpts = Array.isArray(options) && options.length > 0 ? options : [
    ...
    showTranslationToast(originalText, translatedText, fromLanguage, toLanguage) {
      const pos = this.getSelectionPosition();
      const toast = document.createElement("div");
      toast.className = "tts-selection-toast";
      toast.style.position = "absolute";
      ...
      titleElem.style.cssText = "font-size:13px;font-weight:700;letter-spacing:-0.02em;color:#111111";
      ...
      inputElemHeader.style.cssText = "color:#111111;font-size:11px;font-weight:600;text-decoration:underline;text-underline-offset:2px;cursor:pointer";
      ...
      origElem.style.cssText = "font-size:11px;font-style:italic;color:#666666;max-height:50px;overflow-y:auto;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;width:100%";
      origElem.textContent = `"${originalText}"`;
      content.appendChild(origElem);
      const outputElemHeader = document.createElement("div");
      outputElemHeader.style.cssText = "color:#111111;font-size:11px;font-weight:600;text-decoration:underline;text-underline-offset:2px;cursor:pointer";
      ...
      transElem.style.cssText = "font-size:13px;font-weight:600;line-height:1.45;color:#111111;max-height:140px;overflow-y:auto;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;width:100%";
      ...
      copyBtn.style.cssText = "font-size:10px;padding:3px 8px;cursor:pointer";
    ...
    setupListeners() {
      chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.action === "speakSelection" && message.text) {
          ...
          return true;
        } else if (message.action === "promptTargetLanguageSelection") {
          this.notificationView.promptTargetLanguage(message.options).then((choice) => {
        ...
        } else if (message.action === "showTranslation" && message.originalText && message.translatedText) {
